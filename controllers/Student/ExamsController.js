...
const getExamRoutine = async (req, res, next) => {
  try {
    ...
    //console.log(sid);
    let queryRes = await pool.query(
      'SELECT co.course_id , et.exam_date \
    ...
    res.status(201).json({ message: "getExamRoutine" , exam_routine_list:exam_routine_list});
  } catch (err) {
...
const getGuidelines = async (req, res, next) => {
  try {
    let queryRes = await pool.query(
      'SELECT * from exam_guidelines where session_id=$1' , [session_id]
    );
    //console.log(queryRes);
